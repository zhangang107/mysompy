...
class View2D(MapView):
    ...
    def build_save(self, som, what='codebook', which_dim='all', cmap=None,
             col_sz=None, desnormalize=False, filename=None):
        (self.width, self.height, indtoshow, no_row_in_plot, no_col_in_plot,
         axis_num) = self._calculate_figure_params(som, which_dim, col_sz)
        self.prepare()
...
class View2DPacked(MapView):
    ...
    def buidl_save(self, som, what='codebook', which_dim='all', cmap=None,
             col_sz=None, filename=None):
        if col_sz is None:
            col_sz = 6
        (self.width, self.height, indtoshow, no_row_in_plot, no_col_in_plot,
         axis_num) = self._calculate_figure_params(som, which_dim, col_sz)
